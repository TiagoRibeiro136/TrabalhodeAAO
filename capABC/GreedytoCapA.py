...
def read_data(filename):
    ...
    # Leitura dos armazéns com capacidade substituída por 400000
    for i in range(1, m + 1):
        # Substitui "capacity" por 400000
        data['warehouses'].append({'capacity': 400000, 'fixed_cost': float(lines[i].split()[1]), 'allocated': 0, 'opened': False})
...
def greedy_algorithm(data):
    ...
    for customer_idx, customer in enumerate(customers):
        ...
        if min_warehouse_idx != -1:
            ...
            allocated_demand = min(customer['demand'], warehouses[selected_warehouse]['capacity'])
            warehouses[selected_warehouse]['allocated'] += allocated_demand
            warehouses[selected_warehouse]['capacity'] -= allocated_demand
...
if __name__ == "__main__":
    filename = "FicheirosTeste/ORLIB/capc.txt"
    main(filename)
